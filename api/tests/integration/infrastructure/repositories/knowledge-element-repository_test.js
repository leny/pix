...
describe('Integration | Repository | knowledgeElementRepository', () => {
  ...
  describe('#findValidatedTargetedGroupedByTubesForUsers', () => {
    ...
    it('should return knowledge elements within respective dates grouped by userId and tubeId within target profile of campaign', async () => {
      ...
      // when
      const knowledgeElementsByUserIdAndTubeId =
        await knowledgeElementRepository.findValidatedTargetedGroupedByTubesForUsers({ [userId1]: dateUserId1, [userId2]: dateUserId2 }, targetProfile);

      // then
      expect(knowledgeElementsByUserIdAndTubeId[userId1][tube1.id][0]).to.be.instanceOf(KnowledgeElement);
      expect(knowledgeElementsByUserIdAndTubeId[userId1][tube1.id].length).to.equal(2);
      expect(knowledgeElementsByUserIdAndTubeId[userId1][tube1.id]).to.deep.include.members([knowledgeElement1_1, knowledgeElement1_2]);
      expect(knowledgeElementsByUserIdAndTubeId[userId2]).to.deep.equal({
        [tube1.id]: [knowledgeElement2_1],
        [tube2.id]: [knowledgeElement2_2],
      });
    });
    ...
    it('should return the knowledge elements in the snapshot when user has a snapshot for this date', async () => {
      ...
      const knowledgeElement = databaseBuilder.factory.buildKnowledgeElement({
        userId,
        competenceId: targetProfile.tubes[0].id,
        skillId: targetProfile.skills[0].id,
      });
      databaseBuilder.factory.buildKnowledgeElementSnapshot({ userId, snappedAt: dateUserId, snapshot: JSON.stringify([knowledgeElement]) });
      await databaseBuilder.commit();
      ...
      // when
      const knowledgeElementsByUserIdAndTubeId =
        await knowledgeElementRepository.findValidatedTargetedGroupedByTubesForUsers({ [userId]: dateUserId }, targetProfile);

      // then
      expect(knowledgeElementsByUserIdAndTubeId[userId][targetProfile.tubes[0].id][0]).to.deep.equal(knowledgeElement);
    });
    ...
    context('when user does not have a snapshot for this date', () => {
      ...
      context('when no date is provided along with the user', () => {
        ...
        it('should return the knowledge elements with limit date as now', async () => {
          ...
          // when
          const knowledgeElementsByUserIdAndTubeId =
            await knowledgeElementRepository.findValidatedTargetedGroupedByTubesForUsers({ [userId]: null }, targetProfile);

          // then
          expect(knowledgeElementsByUserIdAndTubeId[userId]).to.deep.equal({
      ...
      context('when a date is provided along with the user', () => {
        ...
        it('should return the knowledge elements at date', async () => {
          ...
          // when
          const knowledgeElementsByUserIdAndTubeId =
            await knowledgeElementRepository.findValidatedTargetedGroupedByTubesForUsers({ [userId]: new Date('2018-02-01') }, targetProfile);
