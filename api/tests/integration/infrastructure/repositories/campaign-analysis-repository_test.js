  ...
  describe('#getCampaignAnalysis', () => {
    ...
    context('in a rich context close to reality', () => {
      ...
      beforeEach(async () => {
        ...
        const url1 = domainBuilder.buildTargetedSkill({ id: 'recUrl1', tubeId: 'recTubeA', name: '@url1' });
        const tubeUrl = domainBuilder.buildTargetedTube({ id: 'recTubeA', competenceId: 'recCompetence', skills: [url1] });

        const file2 = domainBuilder.buildTargetedSkill({ id: 'recFile2', tubeId: 'recTubeB', name: '@file2' });
        const tubeFile = domainBuilder.buildTargetedTube({ id: 'recTubeB', competenceId: 'recCompetence', skills: [file2] });
      ...
      context('when there is a single participant who shared its contribution', () => {
        ...
        beforeEach(() => {
          ...
          _.each([
            ...
            { userId: fredId, skillId: 'recFile2', status: 'invalidated', campaignId, createdAt: beforeCampaignParticipationShareDate },
        ...
        it('should resolves an analysis based on participant score ignoring untargeted or non validated knowledge elements', async () => {
          ...
          // then
          const tubeARecommendation = actualAnalysis.campaignTubeRecommendations[0];
          expect(tubeARecommendation.averageScore).to.equal(15);
          const tubeBRecommendation = actualAnalysis.campaignTubeRecommendations[1];
          expect(tubeBRecommendation.averageScore).to.equal(30);
        });
      });
